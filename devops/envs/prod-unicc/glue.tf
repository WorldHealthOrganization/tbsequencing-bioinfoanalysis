...
locals {
  glue_connection = {
    glue_connection = {
      ...
      subnet_id              = data.aws_subnets.private-a.ids[0]
...
locals {
  glue_jobs = {
    ...
    deletion = {
      ...
      script_location = "s3://${local.glue_jobs_bucket}/glue-jobs/deletion"
    }
    locus_stats = {
      role_arn          = aws_iam_role.glue_role.arn
      connections       = [module.glue.glue_connection_name["glue_connection"]]
      ...
      script_location = "s3://${local.glue_jobs_bucket}/glue-jobs/locus-stats"
    }
    taxonomy_assignment = {
      role_arn          = aws_iam_role.glue_role.arn
      connections       = [module.glue.glue_connection_name["glue_connection"]]
    ...
    global_stats = {
      ...
      script_location = "s3://${local.glue_jobs_bucket}/glue-jobs/global-stats"
    }
    del_variants = {
      role_arn          = aws_iam_role.glue_role.arn
      connections       = [module.glue.glue_connection_name["glue_connection"]]
    ...
    join_genotype = {
      ...
      script_location = "s3://${local.glue_jobs_bucket}/glue-jobs/join-genotype"
    }
    predict_resistance = {
      role_arn          = aws_iam_role.glue_role.arn
      connections       = [module.glue.glue_connection_name["glue_connection"]]
    ...
    write_formatted_annotations_per_gene = {
      ...
      default_arguments = {
        "--job-bookmark-option"      = "job-bookmark-enable",
        "--enable-spark-ui"          = "true",
        "--extra-py-files"           = "s3://${local.glue_jobs_bucket}/glue-jobs/variant_annotation_categorization.py,s3://${local.glue_jobs_bucket}/glue-jobs/biosql_gene_views.py",
